...
The median value is **`r format(stepStats['Median'])`**  
...
maxtime<-paste(hour(hiActiv),
               ...
               sep=":")
```
The 5minute interval in which the maximum average number of steps was recorded is **`r  maxtime`** with an average value of `r format(maxit$steps,digits=4)` .
...
# logical vector of NA values 
   ...
   numNAs<-sum(NA_yes)
```
The total number of NAs in the data set is `r numNAs`  
...
print(stepStats2)
```
 `r paste(names(c(stepStats)), collapse="|")`   
 `r paste(format(stepStats,digits=5), collapse="|")`   
 *Dataset following imputation of NAs*  
 `r paste(format(stepStats2,digits=5),collapse="|")`  
